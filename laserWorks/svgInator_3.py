...
class svgImage(svgShape):
    def __init__(self, filename, width, height, pos=vector(0,0,0), style={}, units="mm"):
    ...
    def getText(self):
        self.txt = f'<image href="{self.filename}" height="{self.height}" width="{self.width}" x="{self.pos.x}" y="{self.pos.y}"/>'
...
class svgInator:
    ...
    def image(self, filename, pos=vector(0,0,0), style={}):
        import matplotlib.image as pltimg
        imgArray = pltimg.imread(filename)
        (width, height, color) = imgArray.shape
        img = svgImage(filename, width=width, height=height, pos=pos,  style=style)
